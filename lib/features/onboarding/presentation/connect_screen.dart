...
class ConnectScreen extends ConsumerStatefulWidget {
  const new({this.preset, super.key});
...
class _ConnectScreenState extends ConsumerState<ConnectScreen> {
  ...
  Map<DraftField, DraftProblem> _problems(SourceDraft draft) =>
      validateDraft(draft, requirePassword: true);
  ...
  Future<void> _test() async {
    ...
    final result = await ref.read(sourceCheckerProvider).check(draft);
  ...
  /// Enter in any field: test, or start once the test has passed.
  void _submit() => unawaited(_check is CheckPassed ? _start() : _test());
  ...
  Future<void> _back() async {
    ...
    context.go(any ? '/' : welcomeRoutePath);
  }
  ...
  void _focusField(DraftField field) {
    ...
    if (field == DraftField.name || field == DraftField.epgUrl) {
          ...
          children: [
            ...
            _AdvancedToggle(
              ...
              label: switch (_type) {
                ...
                SourceType.m3uUrl => 'Advanced: name, User-Agent, guide link',
                SourceType.m3uFile => 'Advanced: name, guide link',
              },
