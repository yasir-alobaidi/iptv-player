...
- **Retries:** 429 and 5xx back off 1 s, 2 s (±20 %), or `Retry-After` capped at 30 s, three attempts in all; a body cut off or silent for 30 s mid-transfer is retried too. A refused connection is not retried, and the onboarding check (`account(retry: false)`) never retries — the user is waiting. 401 and 403 are auth failures; 404 is not found, **except an empty 404, which is a refused sign-in** (next point).
- **Checked against the user's real panel (2026-09-19, nginx + PHP 7.4):** a wrong username or password is not `200` with `auth: 0` but **`404` with an empty body** (`Content-Type: application/json`, `X-Powered-By: PHP`), for every action; a page that really isn't there gets nginx's own 146-byte error page. So an empty (whitespace-only) 404 is an `AuthFailure`, a 404 with a body stays a `NotFoundFailure` ("Not found"); both carry the status, and the UI shows the server's answer beside its own reading ("The server answered HTTP 404 with an empty page, which is how some panels refuse a wrong username or password."), and the fake provider has the `refusedSignInAs404` quirk (on in `quirky`). Everything else matched this document: 379 categories, 12,610 channels, 20,072 movies and 8,262 series parsed with no row dropped and no warning, a first sync in 8.7–9.4 s and a re-sync in 7.8 s; `server_info.timezone` is an IANA name (`Africa/Cairo`); `allowed_output_formats` was `["m3u8","ts"]`; `max_connections` was **1**, already in use by another device, which Phase 3 must say plainly rather than fail to play.
...
| sync_runs | id, source_id, started_at, finished_at, outcome (running/succeeded/failed/cancelled), failure (an `AppFailure.code`), failure_status (the HTTP status the server answered, v3), counts_json — the id is the mark-and-sweep marker. Never the failure's detail text: it can carry a credential |
...
- An Xtream list that comes back empty keeps the previous one (and its categories, with the user's hidden flags) instead of sweeping it, unless the previous successful run found it empty too; an M3U playlist with no entries fails the run
- Removing a source stops its sync first (`SyncService.removeSource`), then deletes the source with its catalogue
