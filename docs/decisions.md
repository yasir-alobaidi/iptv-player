...
- **An Xtream list that comes back empty keeps the last one**, and its categories with the user's hidden flags: an empty `get_vod_streams` is far more often a panel's hiccup than a provider that dropped every movie. It is logged. Known limit: a provider that really drops a whole kind leaves it in place.
...
- **For step 7:** removing a source while it syncs isn't coordinated yet — its next batch fails on the foreign key and the run ends failed. Settings → Sources should cancel the sync first.
